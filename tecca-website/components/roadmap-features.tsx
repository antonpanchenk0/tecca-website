...
const initialManagedPosts: ManagedPost[] = [
  {
    ...
    description: 'Add dark mode toggle and theme system for the entire app',
    status: 'in-progress',
    priority: 'high',
    product: 'web-app',
    board: 'feature-request',
    votes: 24,
    tags: ['Web Version', 'No prod'],
    comments: 18,
    date: 'Sep 18',
  },
  {
...
export default function RoadmapFeaturesSection() {
  ...
  const handlePriorityChange = (postId: number, newPriority: ManagedPost['priority']) => {
    setManagedPosts((prev) =>
      prev.map((p) => (p.id === postId ? { ...p, priority: newPriority } : p)),
    );
  };

  const handleProductChange = (postId: number, newProduct: ManagedPost['product']) => {
    setManagedPosts((prev) =>
      prev.map((p) => (p.id === postId ? { ...p, product: newProduct } : p)),
    );
  };
                          ...
                          {columns.map((column) => {
                            ...
                            return (
                              ...
                              >
                                {/* Column Header */}
                                <div
                                  className={`flex items-center justify-between px-3 py-1.5 border-b border-border ${column.headerBg}`}
                                >
                  ...
                  {activeTab === 2 && (
                    <div className="space-y-4 flex-1 flex flex-col">
                      <div className="w-full flex-col gap-2 text-center">
                        ...
                        <p className="text-muted-foreground">
                          Manage posts straight from the roadmap view. Click on a specific post on
                          the list or roadmap to manage it directly from the chosen view
                        </p>
                      </div>
                      ...
                      <div className="w-full overflow-x-auto lg:overflow-x-visible">
                        <div className="flex gap-3 md:gap-4 pb-2 lg:justify-center">
                          {(Object.keys(statusConfig) as ManagedPost['status'][]).map(
                            (statusKey) => {
                              const config = statusConfig[statusKey];
                              const columnPosts = managedPosts.filter(
                              ...
                              return (
                                <div
                                  key={statusKey}
                                  className="w-[260px] md:w-[280px] lg:w-[300px] flex-shrink-0 bg-muted/30 rounded-lg border border-border flex flex-col overflow-visible"
                                >
                                  {/* Column Header */}
                                  <div
                                    className={`flex items-center justify-between px-3 py-1.5 border-b border-border ${config.bgColor}`}
                                  >
                                    <div className="flex items-center gap-1.5">
                                      <div
                                        className={
                                          columns.find((c) => c.id === statusKey)?.textColor
                                        }
                                      >
                                            ...
                                            <div className="flex items-center justify-between">
                                              ...
                                              <div className="relative">
                                                ...
                                                >
                                                  {config.label}
                                                  <svg
                                                    className={`w-1.5 h-1.5 transition-transform ${openDropdown === `status-${post.id}` ? 'rotate-180' : ''}`}
                                                    fill="none"
                                                    stroke="currentColor"
                                                    viewBox="0 0 24 24"
                                                  >
                                                      ...
                                                      {(
                                                        ...
                                                        <button
                                                          ...
                                                          className={`w-full text-left text-[10px] px-2.5 py-1.5 transition-colors cursor-pointer flex items-center gap-2 ${
                                                            post.status === s
                                                              ? `${statusConfig[s].bgColor} ${statusConfig[s].textColor} font-medium`
                                                              : 'text-foreground hover:bg-muted dark:hover:bg-[#24252B]'
                                                          }`}
                                                        >
                                                          <div
                                                            className={`w-2 h-2 rounded-full flex-shrink-0 ${statusConfig[s].color}`}
                                                          />
                                            ...
                                            <div className="flex items-center gap-1 flex-wrap">
                                              ...
                                              <div className="relative">
                                                ...
                                                >
                                                  ...
                                                  <svg
                                                    className={`w-1.5 h-1.5 transition-transform ${openDropdown === productDropdownKey ? 'rotate-180' : ''}`}
                                                    fill="none"
                                                    stroke="currentColor"
                                                    viewBox="0 0 24 24"
                                                  >
                                                  ...
                                                  {openDropdown === productDropdownKey && (
                                                    ...
                                                    >
                                                      {(
                                                        Object.keys(
                                                          productConfig,
                                                        ) as ManagedPost['product'][]
                                                      ).map((product) => (
                                                        <button
                                                          key={product}
                                                          onClick={(e) => {
                                                            e.stopPropagation();
                                                            handleProductChange(post.id, product);
                                                            setOpenDropdown(null);
                                                          }}
                                                          className={`w-full text-left text-[10px] px-2.5 py-1.5 transition-colors cursor-pointer ${
                                                            post.product === product
                                                              ? `${productConfig[product].bgColor} ${productConfig[product].color} font-medium`
                                                              : 'text-foreground hover:bg-muted dark:hover:bg-[#24252B]'
                                                          }`}
                                                        >
                                              ...
                                              <div className="relative">
                                                ...
                                                >
                                                  ...
                                                  <svg
                                                    className={`w-1.5 h-1.5 transition-transform ${openDropdown === boardDropdownKey ? 'rotate-180' : ''}`}
                                                    fill="none"
                                                    stroke="currentColor"
                                                    viewBox="0 0 24 24"
                                                  >
                                                  ...
                                                  {openDropdown === boardDropdownKey && (
                                                    ...
                                                    >
                                                      {(
                                                        Object.keys(
                                                          boardConfig,
                                                        ) as ManagedPost['board'][]
                                                      ).map((board) => (
                                                        <button
                                                          key={board}
                                                          onClick={(e) => {
                                                            e.stopPropagation();
                                                            handleBoardChange(post.id, board);
                                                            setOpenDropdown(null);
                                                          }}
                                                          className={`w-full text-left text-[10px] px-2.5 py-1.5 transition-colors cursor-pointer ${
                                                            post.board === board
                                                              ? `${boardConfig[board].bgColor} ${boardConfig[board].color} font-medium`
                                                              : 'text-foreground hover:bg-muted dark:hover:bg-[#24252B]'
                                                          }`}
                                                        >
                                              ...
                                              <div className="relative">
                                                ...
                                                >
                                                  ...
                                                  <svg
                                                    className={`w-1.5 h-1.5 transition-transform ${openDropdown === priorityDropdownKey ? 'rotate-180' : ''}`}
                                                    fill="none"
                                                    stroke="currentColor"
                                                    viewBox="0 0 24 24"
                                                  >
                                                  ...
                                                  {openDropdown === priorityDropdownKey && (
                                                    ...
                                                    >
                                                      {(
                                                        Object.keys(
                                                          priorityConfig,
                                                        ) as ManagedPost['priority'][]
                                                      ).map((priority) => (
                                                        <button
                                                          key={priority}
                                                          onClick={(e) => {
                                                            e.stopPropagation();
                                                            handlePriorityChange(post.id, priority);
                                                            setOpenDropdown(null);
                                                          }}
                                                          className={`w-full text-left text-[10px] px-2.5 py-1.5 transition-colors cursor-pointer ${
                                                            post.priority === priority
                                                              ? `${priorityConfig[priority].bgColor} ${priorityConfig[priority].color} font-medium`
                                                              : 'text-foreground hover:bg-muted dark:hover:bg-[#24252B]'
                                                          }`}
                                                        >
